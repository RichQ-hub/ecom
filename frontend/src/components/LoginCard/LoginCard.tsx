 ...
 * LoginCard Component
 *
 * This component provides a user interface for logging into the application. It includes input fields for the user's email and password,
 * and a button to submit the login form. It also includes functionality for showing/hiding the password, email validation, and handling
 * login submission.
 *
 */
const LoginCard = () => {
   ...
   * Navigates to the companies page if login is successful else displays an error message.
   */
      ...
      <div className="bg-auth-modal-bg rounded-xl mx-auto px-8 max-w-lg font-title">
        <div className="flex min-h-full flex-1 flex-col justify-center py-6">
          <div className="mt-5 sm:mx-auto sm:w-full sm:max-w-sm">
            {/* Error message display if unsuccessful login attempt */}
            {errorMessage && (
            ...
            <form className="space-y-6" onSubmit={handleLoginSubmit}>
              ...
              <div>
                ...
                {!isEmailValid && <p className="text-red-500 mt-1">Please enter a valid email address</p>}{' '}
              </div>
              ...
              <div>
                <button
                  ...
                  className={`flex w-full justify-center rounded-md py-1.5 text-xl font-semibold text-black shadow-sm hover:bg-ecom-light-blue-900 ${!isEmailValid || email.length === 0 || password.length === 0 ? 'bg-gray-300 cursor-not-allowed opacity-50' : 'bg-ecom-light-blue'} transition duration-150 ease-in-out`}
                >
                  Log in
                </button>
              </div>
