import React, { useEffect, useState } from 'react';
import Modal from '../Modal';
import useFormInputText from '../../hooks/useFormInputText';
import { MetricDetails } from '../../types/metric';
import MetricService from '../../services/MetricService';
import { CircularProgress } from '@mui/material';
import clsx from 'clsx';

interface SelectMetricModalProps {
  handleCloseModal: () => void;
  onSelectMetric: (metric: MetricDetails) => void;
  activeCategory: string;
}

const SelectMetricModal: React.FC<SelectMetricModalProps> = ({
  handleCloseModal,
  onSelectMetric,
  activeCategory
}) => {
  const [metrics, setMetrics] = useState<MetricDetails[]>([]);
  const [selectedMetric, setSelectedMetric] = useState<MetricDetails>();
  const [loading, setLoading] = useState<boolean>(false);
  const query = useFormInputText();

  const getCategoryAbbreviation = (categoryName: string): 'E' | 'S' | 'G' => {
    switch (categoryName) {
      case 'Environmental':
        return 'E';
      case 'Social':
        return 'S';
      case 'Governance':
        return 'G';
      default:
        throw new Error(`Unknown category: ${categoryName}`);
    }
  };

  useEffect(() => {
    setLoading(true);
    const fetchData = async () => {
      const metricsResponse = await MetricService.obtainCategoryMetrics(getCategoryAbbreviation(activeCategory));
      setMetrics(metricsResponse);
    }
    fetchData();
    setLoading(false);
  }, []);

  const handleSubmitQuery = async (e: React.FormEvent) => {
    setLoading(true);
    e.preventDefault();

    const metricsResponse = await MetricService.obtainCategoryMetrics(getCategoryAbbreviation(activeCategory));
    setMetrics(metricsResponse);
    setLoading(false);
  }

  const handleSelectMetric = () => {
    if (selectedMetric) {
      onSelectMetric(selectedMetric); // Pass selected metric
    }
    handleCloseModal();
  }

  return (
    <Modal
      title='Select Metric'
      handleCloseModal={handleCloseModal}
    >
      {/* Search bar */}
      <form
        className='mb-4 flex items-center border-black border-[1px] rounded-lg'
        action=''
        onSubmit={handleSubmitQuery}
      >
        <label className='hidden' htmlFor='metric-search-bar'>Metric Search Bar</label>
        <input
          className='w-full h-12 px-6 text-base bg-transparent outline-none'
          placeholder='Metric Name...'
          name='metric-search-bar'
          id='metric-search-bar'
          type='text'
          onChange={query.handleChange}
          value={query.value}
        />
        <button
          className='w-12 p-3 rounded-full hover:bg-zinc-300'
          type='submit'
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z"/></svg>
        </button>
      </form>

      {/* Metric List */}
      {loading ? (
        <CircularProgress className='mx-auto' />
      ) : (
        <ul className='mb-4 bg-white max-h-56 overflow-y-scroll scrollbar'>
          {metrics.map((f) => {
            return (
              <li className='block' key={f.metric_id}>
                <button
                  className={clsx(
                    'text-left w-full px-4 py-2 hover:bg-slate-300',
                    {
                      'bg-slate-200': f.metric_id === selectedMetric?.metric_id
                    }
                  )}
                  type='button'
                  onClick={() => setSelectedMetric(f)}
                >
                  {f.name}
                </button>
              </li>
            )
          })}
        </ul>
      )}

      {/* Apply Button */}
      <button
        className='block ml-auto bg-ecom-modal-submit-btn border-[1px] border-black py-2 px-4 hover:brightness-110 disabled:brightness-50 disabled:cursor-not-allowed'
        type='button'
        disabled={!selectedMetric}
        onClick={handleSelectMetric}
      >
        Apply
      </button>
    </Modal>
  )
}

export default SelectMetricModal;
